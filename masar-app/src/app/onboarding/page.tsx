...
}: { skill: SkillListItem; selected: boolean; onToggle: (id: number) => void }) {
  return (
    <button
      onClick={() => onToggle(skill.id)}
      className={`rounded-full px-3.5 py-1.5 text-[13px] font-semibold transition-colors ${
        selected ? 'bg-primary text-white' : 'border border-black/[0.12] bg-white text-text'
      }`}
    >
      {skill.name}
    </button>
...
export default function OnboardingPage() {
  ...
  // Pre-fetch skills one step early so they're ready when step 4 opens
  useEffect(() => {
  ...
  async function handleNext() {
    ...
    try {
      ...
      try {
        ...
        if (msg.includes('400')) {
          setLoading(false)
          setRegisterError('هذا البريد الإلكتروني مسجل بالفعل — سجّل دخولك من صفحة تسجيل الدخول')
          return
        }
        // unexpected error — bail out
        setLoading(false)
        setRegisterError('حدث خطأ أثناء إنشاء الحساب، حاول مجدداً')
        return
      }
  ...
  const filteredSkills = useMemo(() => {
    ...
    if (!q) return skills.slice(0, 60) // show first 60 until user searches
    return skills.filter(s => s.name.toLowerCase().includes(q)).slice(0, 60)
  }, [skills, skillSearch])

  const skillsByCategory = useMemo(() =>
    filteredSkills.reduce<Record<string, SkillListItem[]>>((acc, s) => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-canvas px-5">
        ...
        {step === 4 && (
          <div className="space-y-4 pb-4">
            ...
            <input
              ...
              className="w-full rounded-xl border border-black/[0.12] bg-white px-4 py-3 text-right text-[14px] text-text placeholder:text-muted/60 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
            ...
            {skills.length === 0 ? (
              ...
              <>
                ...
                {!skillSearch && skills.length > 60 && (
                  <p className="text-center text-[12px] text-muted">
                    ابحث لعرض المزيد من المهارات ({skills.length - 60} مهارة إضافية)
                  </p>
                )}
              </>
            )}
      ...
      <div className="sticky bottom-0 bg-canvas pb-8 pt-4">
        {registerError && (
          <p className="mb-3 rounded-xl bg-red-50 px-4 py-3 text-right text-[13px] font-semibold text-red-600">
            {registerError}
          </p>
        )}
        {step === 4 && (
