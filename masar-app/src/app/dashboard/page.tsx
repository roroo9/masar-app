...
export default function Dashboard() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      const [d, s] = await Promise.all([getDashboard(sid), getStudentSkills(sid)]);
      setDashboard(d);
      setSkills(s);
      setStatus("ready");

      // Phase 2 — readiness in background
      const topJobId = d.top_scores[0]?.job_id ?? d.jobs[0]?.job_id;
      if (topJobId != null) {
        setReadinessLoading(true);
        getReadiness(sid, topJobId)
            ...
            <div className="rounded-2xl bg-primary p-5 text-white shadow-[0_10px_30px_-12px_rgba(93,63,211,0.45)]">
              {readinessLoading ? (
                <div className="flex flex-col items-center py-6 gap-3">
                  <div className="h-[160px] w-[160px] rounded-full border-[10px] border-white/20 animate-pulse" />
                  <div className="h-4 w-48 rounded-full bg-white/20 animate-pulse" />
                  <div className="h-3 w-36 rounded-full bg-white/15 animate-pulse" />
                </div>
          ...
          <section className="mt-5 px-5">
            ...
            <div className="grid grid-cols-3 gap-2.5">
              ...
              <div className="rounded-xl border border-black/[0.06] bg-white p-3 text-center">
                ...
                {readinessLoading ? (
                  <div className="mt-1 mx-auto h-7 w-8 rounded bg-black/8 animate-pulse" />
                ) : (
                  <p className="mt-1 text-[22px] font-bold text-text">
