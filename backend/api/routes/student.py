...
class AddExtractedSkills(BaseModel):
    skill_ids: List[int]


def _extract_skills_preview(text: str, conn) -> list:
...
def extract_from_description(student_id: OwnStudent, data: CourseDescriptionExtract):
    ...
    with pooled_connection() as conn:
        skills_out = _extract_skills_preview(text, conn)
        conn.commit()
...
async def extract_from_pdf(student_id: OwnStudent, file: UploadFile = File(...)):
    ...
    with pooled_connection() as conn:
        skills_out = _extract_skills_preview(full_text, conn)
        conn.commit()
    return {"skills": skills_out, "page_count": len(text_parts)}


@router.post("/{student_id}/add-extracted-skills")
def add_extracted_skills(student_id: OwnStudent, data: AddExtractedSkills):
