...
def create_student(student: StudentCreate):
    ...
    """, (
        ...
        student.major, student.year_of_study,
        student.university
    ))
...
def add_student_courses(student_id: int, data: CourseAdd):
    conn = get_connection()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    ...
    if added:
        # Courses changed — invalidate cached readiness so next fetch recomputes
        cursor.execute(
            "DELETE FROM readiness_scores WHERE student_id = %s", (student_id,)
        )
...
def get_student_skills_route(student_id: int):
    conn = get_connection()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)

    # Course-derived skills (no embedding filter — display only)
    cursor.execute("""
        SELECT DISTINCT
    ...
    # Self-reported / extra skills (no embedding filter)
    cursor.execute("""
        SELECT
            s.id, s.name, s.category,
            (ses.proficiency / 5.0) as confidence,
            ses.source
    ...
    # Merge: extra skills win on conflict (preserves self_reported source)
    all_skills: dict = {}
    ...
    def fmt(s):
        return {"id": s['id'], "name": s['name'], "confidence": float(s['confidence'] or 0), "source": s.get('source') or 'course'}
...
def get_dashboard(student_id: int):
    ...
    # Get skill count (courses + extra skills)
    cursor.execute("""
        SELECT COUNT(*) FROM (
    ...
    for job in all_jobs:
        jobs_with_scores.append({
            ...
            "readiness_score": scores_dict.get(job['id'], None)
        })
    ...
    return {
        ...
        "top_scores": [dict(s) for s in scores[:3]],
        "jobs": jobs_with_scores
    }
@router.post("/{student_id}/extra-skills")
def add_extra_skills(student_id: int, data: BulkSkillAdd):
    conn = get_connection()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    ...
    if data.skill_ids:
        cursor.execute(
            "SELECT id FROM skills WHERE id = ANY(%s)", (data.skill_ids,)
        )
    ...
    if added:
        # Skills changed — invalidate cached readiness so next fetch recomputes
        cursor.execute(
            "DELETE FROM readiness_scores WHERE student_id = %s", (student_id,)
        )
...
def get_student_projects(student_id: int, job_id: Optional[int] = None, limit: int = 5):
    from core.recommender import recommend_projects
    projects = recommend_projects(student_id, job_id, limit)
...
def confirm_skill(student_id: int, data: ConfirmSkill):
    conn = get_connection()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)

    # Look up skill by name (case-insensitive)
    cursor.execute(
        "SELECT id FROM skills WHERE LOWER(name) = LOWER(%s)", (data.skill_name,)
    )
    row = cursor.fetchone()
...
def extract_from_description(student_id: int, data: CourseDescriptionExtract):
...
async def extract_from_pdf(student_id: int, file: UploadFile = File(...)):
